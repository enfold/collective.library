...
define([
  ...
  var InlineValidation = Base.extend({
    ...
    validate_file_field: function (input) {
        var $input = $(input),
            $field = $input.closest('.field'),
            $form = $field.closest('form'),
        ...
        if (fname) {
            this.queue($.proxy(function(next) {
                $.ajax({
                    type: 'POST',
                    url: this.append_url_path($form.attr('action'), '@@z3cform_validate_file_field'),
                    data: {fname: fname, filename: filename},
                    iframe: false,
                    success: $.proxy(function (data) {
                      this.render_error($field, data.errmsg);
                      next();
    ...
    init: function () {
      ...
      this.$el.find('input[type="file"]').on('change',
          $.proxy(function (ev) {
            this.validate_file_field(ev.target);
          }, this));
      },
  });
